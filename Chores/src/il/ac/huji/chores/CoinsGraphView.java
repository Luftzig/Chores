...
/**
 * User: Yoav Luft Email: Yoav.luft@gmail.com Date: 14/09/13
 */
public class CoinsGraphView extends View {

	private ShapeDrawable[] bars;
	// Width and Height use UPPERCASE to signify that they are to be treated as
	// constants
	...
	private void initialize() {
		try {
			...
			roommatesNames = ApartmentDAL.getApartmentRoommates(RoommateDAL
					.getApartmentID());
			this.roommates = getRoommates(roommatesNames);
		} catch (UserNotLoggedInException e) {
			LoginActivity.OpenLoginScreen(this.getContext(), false);
			...
			if (e.getCode() == ParseException.CONNECTION_FAILED) {
				Toast.makeText(
						...
						this.getContext().getResources()
								.getString(R.string.chores_connection_failed),
		...
		/*
		 * int numBars = roommates.size() > 0? roommates.size() : 1; int
		 * horizontalSize = WIDTH / numBars / 2; int horizontalSpace = WIDTH /
		 * numBars / 4; bars = new ShapeDrawable[numBars]; bars[0] = new
		 * ShapeDrawable(new RectShape()); bars[0].setBounds(horizontalSpace, 0,
		 * horizontalSize, HEIGHT); bars[0].getPaint().setColor(0xff000000);
		 */
	...
	private List<Roommate> getRoommates(List<String> roommates) throws ParseException 
			{
