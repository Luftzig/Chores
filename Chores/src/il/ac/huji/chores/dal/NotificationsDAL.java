...
public class NotificationsDAL {
	...
	public static void notifyChoreDone(Chore chore, String sender,
		...
		pushQuery.whereEqualTo("channels", PARSE_DONE_CHANNEL_KEY.toString()); // Set
																				// the
																				// channel
	...
	public static void notifyChoreMissed(Chore chore, String sender,
		...
		// Create our Installation query
		ParseQuery<ParseInstallation> pushQuery = ParseInstallation.getQuery();
		pushQuery.whereEqualTo("channels", PARSE_MISSED_CHANNEL_KEY.toString()); // Set
																					// the
																					// channel
		pushQuery.whereContainedIn("username", roommates);
	...
	public static void notifySuggestStealChore(Chore chore, String sender,
		...
		// Create our Installation query
		ParseQuery<ParseInstallation> pushQuery = ParseInstallation.getQuery();
		pushQuery.whereEqualTo("channels", PARSE_STEAL_CHANNEL_KEY.toString()); // Set
																				// the
																				// channel
		pushQuery.whereContainedIn("username", roommates);
	...
	public static void notifySuggestChore(Chore chore, String sender,
		...
		ParseQuery<ParseInstallation> pushQuery = ParseInstallation.getQuery();
		pushQuery
				.whereEqualTo("channels", PARSE_SUGGEST_CHANNEL_KEY.toString()); // Set
																					// the
																					// channel
		pushQuery.whereContainedIn("username", roommates);
		String msg = sender + " suggest you to take the chore :"
				+ chore.getName();
		JSONObject data = createDataJson(msg,
				PARSE_SUGGEST_CHANNEL_KEY.toString());
		try {
	...
	public static void notifySuggestChoreAccepted(Chore chore, String sender,
		...
		ParseQuery<ParseInstallation> pushQuery = ParseInstallation.getQuery();
		pushQuery.whereContainedIn("username", roommates);
		String msg = sender + " accepted to take your chore : "
				+ chore.getName();
		JSONObject data = createDataJson(msg,
				PARSE_SUGGEST_ACCEPTED_CHANNEL_KEY.toString());
		// Send push notification to query
		ParsePush push = new ParsePush();
		push.setQuery(pushQuery);
		push.setData(data);
	...
	public static void notifyInvitationAccepted(String accepter,
		...
		Log.d("NotificationsDAL.notifyInvitationAccepted",
				"Accepted message will be sent to " + roommates.toString());
		ParseQuery<ParseInstallation> query = ParseInstallation.getQuery();
		query.whereEqualTo("channels", PARSE_JOINED_CHANNEL_KEY.toString()); // Set
																				// the
																				// channel
